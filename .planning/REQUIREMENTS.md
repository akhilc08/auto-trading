...
- [ ] **SCHEMA-04**: System creates `daily_pnl` table with columns: `date` (PK with strategy_name + account_name), `strategy_name`, `account_name`, `realized_pnl`, `trade_count`, `win_count`, `sharpe_7d`, `max_drawdown`
...
- [ ] **SCHEMA-10**: Logger degrades gracefully when `MOTHERDUCK_TOKEN` is absent — local runs without the env var continue working normally
...
### CI/CD (GitHub Actions — Strategy Execution)

- [ ] **CICD-01**: `.github/workflows/stat_arb.yml` runs strategies under the `stat_arb` account (stat_arb, stat_arb_v2, stat_arb_v3, market_neutral, market_neutral_v2) as matrix jobs on their configured cron schedules
- [ ] **CICD-02**: `.github/workflows/macro_vol.yml` runs strategies under the `macro_vol` account (vol_risk_premium) on its configured cron schedule
- [ ] **CICD-03**: `.github/workflows/trend_following.yml` runs strategies under the `trend_following` account (trend_following, trend_following_v2, multi_factor_equity, multi_factor_equity_v2, regime_switching, post_earnings_drift, rl_alpha, deep_learning, alt_data_fusion) as matrix jobs
- [ ] **CICD-04**: All workflow crons use `timezone: "America/New_York"` and are offset from round hours
- [ ] **CICD-05**: All workflows include `workflow_dispatch` trigger for manual testing
- [ ] **CICD-06**: Matrix jobs use `fail-fast: false` so one strategy failure does not cancel siblings
- [ ] **CICD-07**: Each workflow maps GitHub secrets to env vars explicitly (`MOTHERDUCK_TOKEN`, per-account `ALPACA_API_KEY`, `ALPACA_SECRET_KEY`) — no `.env` file fallback
- [ ] **CICD-08**: Strategies call `alpaca.get_clock().is_open` as a market-hours guard

### Aggregation (MotherDuck Flight)

- [ ] **AGG-01**: A MotherDuck Flight named `daily-pnl-aggregation` is created with a Python `main()` that reads `trades` and writes aggregated rows to `daily_pnl`
- [ ] **AGG-02**: Flight aggregates only `WHERE status = 'filled'` trades for the prior trading day
- [ ] **AGG-03**: Flight is scheduled at 6 PM ET Mon–Fri (`schedule_cron: "0 23 * * 1-5"` UTC summer / `"0 22 * * 1-5"` winter) to allow fill confirmation lag
...
- [ ] **AGG-06**: Flight `access_token_name` is set to a service account token (not personal token)
...
- [ ] **DIVES-04**: Strategy comparison Dive displays Sharpe 7d, max drawdown, win rate %, trade count, and total P&L in a table (not chart) for all strategies side-by-side

## Future Requirements

### v1.1 Candidates

- **SCHEMA-F01**: Data freshness monitoring — alert when `portfolio_snapshots` has no rows for the last trading day (60-day GitHub Actions auto-disable mitigation)
...
- **DIVES-F03**: Strategy correlation heatmap (high SQL complexity)
- **DIVES-F04**: Drawdown recovery chart
- **AGG-F01**: Migrate Flight to native MotherDuck scheduled queries UI when available

## Out of Scope

| Feature | Reason |
|---------|--------|
| Alpaca keys in Flight config | Flight config is unencrypted — keys stay in GitHub Actions secrets only |
| Dedicated server / container | GitHub Actions handles strategy execution at no cost |
| Real-time streaming dashboards | Alpaca WebSocket → MotherDuck path requires separate infra; Dives refresh on schedule |
...
Filled by roadmapper after ROADMAP.md is created.
